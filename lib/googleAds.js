...
export function getCustomer(client, customerId, refreshToken) {
  return client.Customer({
    ...
    login_customer_id: customerId, // IMPORTANT: This was missing!
...
export async function uploadClickConversion({
  ...
  try {
    console.log('Uploading conversion using google-ads-api library (CORRECT METHOD)...')
    ...
    // Initialize client and customer
    ...
    // customer.conversionUploads.uploadClickConversions(conversionsArray, options)
    const result = await customer.conversionUploads.uploadClickConversions(
      [conversion], // Array of conversions
      { partial_failure: true } // Options object
    )
