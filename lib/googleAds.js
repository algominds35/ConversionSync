...
export async function uploadClickConversion({
  ...
  try {
    ...
    for (const version of apiVersions) {
      const apiUrl = `https://googleads.googleapis.com/${version}/customers/${customerId}:uploadClickConversions`
      ...
      // If Google returns an HTML 404 page, try the next version.
      if (response.status === 404 && contentType.includes('text/html')) {
        console.warn(`Got HTML 404 from ${apiUrl}, trying next API version...`)
