...
export async function uploadClickConversion({
  ...
  try {
    ...
    // Warn if conversionAction customer ID doesn't match customerId
    if (conversionAction && !conversionAction.includes(`customers/${customerId}/`)) {
      console.warn(
        `⚠️ conversionAction does not match customerId. ` +
          `Expected customers/${customerId}/... but got ${conversionAction}`
      )
    }
    ...
    const apiUrl = `https://googleads.googleapis.com/v17/customers/${customerId}:uploadClickConversions`
    const payload = {
