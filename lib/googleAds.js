...
export async function uploadClickConversion({
  ...
  try {
    ...
    // Token diagnostics: verify scopes and audience
    try {
      const tokenInfoResponse = await fetch(
        `https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=${accessToken}`
      )
      const tokenInfoText = await tokenInfoResponse.text()
      console.log('🔍 Tokeninfo status:', tokenInfoResponse.status)
      console.log('🔍 FULL tokeninfo body:', tokenInfoText)
      
      // Parse and check for adwords scope
      try {
        const tokenInfo = JSON.parse(tokenInfoText)
        const hasAdwordsScope = tokenInfo.scope?.includes('adwords')
        console.log('🔍 Has adwords scope:', hasAdwordsScope)
        if (!hasAdwordsScope) {
          console.error('❌ TOKEN MISSING ADWORDS SCOPE!')
          console.error('❌ Current scopes:', tokenInfo.scope)
        }
      } catch (e) {
        console.warn('Could not parse tokeninfo JSON')
      }
    } catch (tokenInfoError) {
      console.warn('Tokeninfo call failed:', tokenInfoError?.message || tokenInfoError)
    }
